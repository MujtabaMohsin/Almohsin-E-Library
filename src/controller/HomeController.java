...
public class HomeController implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		if (db_directory.equals("no DB") || !initializeDB.checkDBexist(db_directory)) {
			...
			isThereDB = false;
			filter_box.getItems().addAll("«”„ «·ﬂ «»", "«”„ «·„ƒ·›", " ÕﬁÌﬁ/≈⁄œ«œ", "„’œ— «·ﬂ «»");
			filter_box.getSelectionModel().selectFirst();
			back_btn.setVisible(false);
			rd_cards.setUserData("»ÿ«ﬁ« ");
			rd_table.setUserData("ÃœÊ·");
	...
	void changePane(ActionEvent event) {
		...
		if (event.getSource() == settings_btn) {
			...
			if (current_showType.equals("»ÿ«ﬁ« ")) {
	...
	public void createCards(String type, String category, int number_of_cards) {
		...
		if (number_of_cards == 0 && type != "category") {
			...
			no_cards_text.setText("⁄–—«ı° Â–« «·ﬁ”„ ·« Ì Ê›— ⁄·Ï ﬂ »");
			...
			if (type.equals("category")) {
				...
				for (int i = 0; i < number_of_cards; i++) {
					...
					label2.setText(categories_data.get(1).get(i) + " ﬂ «»");
	...
	public void createBookPage(String category, int id) {
		...
		if (RetrieveData.checkMyBooks(book_info.get(5), book_info.get(4))) {
			...
			addBook_btn.setText("≈“«·… „‰ ﬂ »Ì");
			addBook_btn.setStyle("-fx-background-color: #FF4136");
		}

		else {

			addBook_btn.setText("«÷› ≈·Ï ﬂ »Ì");
	...
	void update_setting_method(ActionEvent event) {
		...
		if (event.getSource() == update_setting_btn) {
			...
			if (db_path.trim().isEmpty() || folder_path.trim().isEmpty()) {
				...
				error_dialog.setTitle("Œÿ√ »«· ÕœÌÀ");
				error_dialog.setHeight(200);
				error_dialog.setContentText("«·—Ã«¡ «œŒ«· «·»Ì«‰«  »‘ﬂ· ’ÕÌÕ");
				// show the dialog
				error_dialog.show();
				...
				if (!isThereDB) {
					...
					error_dialog.setTitle(" „  ÕœÌÀ «·≈⁄œ«œ« ");
					error_dialog.setHeight(200);
					error_dialog.setContentText(" „  ÕœÌÀ «·≈⁄œ«œ«  »‰Ã«Õ");
					// show the dialog
					error_dialog.show();
	...
	public void settingInitialize() {
		...
		if (!settings_data.isEmpty()) {
			...
			if (settings_data.get(0).equals("ÃœÊ·")) {
				rd_table.setSelected(true);
				current_showType = "ÃœÊ·";
			}

			else {
				rd_cards.setSelected(true);
				current_showType = "»ÿ«ﬁ« ";
			}
		}
	...
	public static void errorDialog() {
		...
		error_dialog.setTitle("Œÿ√ »ﬁ«⁄œ… «·»Ì«‰« ");
		error_dialog.setHeight(200);
		error_dialog.setContentText("Œÿ√ »ﬁ«⁄œ… «·»Ì«‰« ° «·—Ã«¡ «œŒ«· ﬁ«⁄œ… «·»Ì«‰«  «·’ÕÌÕ… √Ê  ÕœÌÀÂ«");
		// show the dialog
		error_dialog.show();
	...
	public void createTabel(String type, String category, int number_of_rows) {
		...
		TableColumn col_book_name = new TableColumn("«”„ «·ﬂ «»");

		TableColumn col_author = new TableColumn("«”„ «·„ƒ·›");
		...
		tabel_books.setPlaceholder(new Label("⁄–—«ı° Â–« «·ﬁ”„ ·« Ì Ê›— ⁄·Ï ﬂ »"));
	...
	public void search(String word, int filter) {
		...
		if (selected_filter.equals("«”„ «·ﬂ «»") && valid_text) {
		...
		else if (selected_filter.equals("«”„ «·„ƒ·›") && valid_text) {
		...
		else if (selected_filter.equals(" ÕﬁÌﬁ/≈⁄œ«œ") && valid_text) {
		...
		else if (selected_filter.equals("„’œ— «·ﬂ «»") && valid_text) {
